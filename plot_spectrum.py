...
def band_heights(x, y):
    """Fitted height of each band, used to normalise the lower panels."""
    out = {}
    for name, spec in ar.BANDS.items():
        base, sigma = ar.linear_baseline(x, y, spec["region"])
        try:
            out[name] = ar.extract_peak(x, y - base, name, sigma)
        except ValueError:
            pass
    return out
...
def main(argv=None) -> int:
    ...
    files = args.files or sorted(
        glob.glob(os.path.join("examples", "synthetic", "*.txt")))
    if not files:
    ...
    for path in files:
        ...
        samples.append((ar.guess_label(path), x, y, band_heights(x, y)))
    ...
    for ax_i, (ax, (lo, hi), region) in enumerate(zip(axes, spans, regions)):
        style(ax)
        for i, (label, x, y, peaks) in enumerate(samples):
            if "G" not in peaks:
                continue
            corr, _, _ = corrected(x, y, region)
            scale = peaks["G"].intensity
            m = (x >= lo) & (x <= hi)
    ...
    # The headline comparison, stated in words.
    lines = []
    for label, x, y, peaks in samples:
        if {"D", "G"} <= peaks.keys():
            lines.append(f"{label}:  ID/IG = {peaks['D'].intensity / peaks['G'].intensity:.2f}")
    if len(lines) == 2:
        fig.text(0.985, 0.005, "   ·   ".join(lines), ha="right", va="bottom",
